...
def walkdir(_path):
  """
  Since the grabdir function is recursive putting the JSON serializer inside
  it created an infinite loop (I think. I DONT KNOW DAMN IT).
  ...
  JSONfile.write(json.dumps(dirlist, indent=2))
  JSONfile.close()

def grabdir(_path):
  """
  Recusively walks a specified directory and grabs all subdirectories
  and files. Files are added as a list with their own key as a child of the
  directory. This took me way too long. I love and hate you python.
  
  Damn it. Remember to catch if the path being specified isnt a directory.
  Do something about that.
  
  Keyword arguments:
  path -- relative path from cwd to the target folder
  
  Returns: A dict of the folder specified in the path containing all
  subfolders and files of all levels.
  """
